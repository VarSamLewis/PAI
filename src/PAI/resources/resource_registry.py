...
class ResourceRegistry:
    ...
    def delete_resource(
        ...
        for i, resource in enumerate(resources_list):
            if (Name and resource.get("Name") == Name) or (ID and resource.get("ID") == ID):
                logger.info(f"Resource found: {resource.get('Name')} (ID: {resource.get('ID')})")
        ...
        try:
            ...
            logger.info(f"Resource deleted successfully. Resources count: {initial_count} → {len(resources_list)}")
            return True
        except Exception as e:
    ...
    def get_resources(
        ...
        if not path.exists():
            logger.debug(f"Resources file not found at {path}, returning empty resources")
            return resources
            ...
            with open(path, "r") as f:
                ...
                if content: 
                    try:
                        ...
                        resources = ResourceCollection.model_validate(resources_raw).model_dump()
                        logger.debug(f"Loaded {len(resources.get('resources', []))} resources")
                    except Exception as validation_error:
                        logger.error(f"Resource validation error: {validation_error}")
                        raise 
                else:
                    logger.debug("Empty resources file, returning default empty structure")
            return resources
            
        except json.JSONDecodeError:
            logger.warning(f"Invalid JSON in resource file: {path}, returning empty resources")
            return resources 
        except Exception as e:
