...
def init(
    ...
    try:
        ...
        if provider not in available_providers:
            typer.echo(f"Provider '{provider}' not supported. Available providers: {', '.join(available_providers)}", err=True)
            raise typer.Exit(1)
...
def status():
    ...
    try:
        ...
        typer.echo(f"   API Key: {'Set' if config.get('api_key') else 'From environment'}")
